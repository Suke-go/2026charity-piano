...
const BASE_DURATION_MS = 9200;
const MIN_GAP_PX = 14;
const MIN_GAP_Y_PX = 22;
const MAX_LANES = 4;
...
export function planFloatingItem(
  item: FloatingItem,
  options: PlanOptions
): { plan: FloatingPlan; reservation: LaneReservation } | null {
  const laneCount = getLaneCount(options.viewportWidth);
  const widthPx = estimateWidth(item.text, options.viewportWidth);
  const heightPx = getItemHeight(options.viewportWidth);
  const durationMs = getDurationMs(widthPx);
  const startY = Math.max(0, options.viewportHeight - Math.max(72, options.viewportHeight * 0.14));
  const endY = Math.max(24, Math.round(options.viewportHeight * 0.1));
...
function getLaneCount(viewportWidth: number) {
  return Math.max(2, Math.min(MAX_LANES, Math.floor(viewportWidth / 180)));
}

function estimateWidth(text: string, viewportWidth: number) {
  const fontSize = viewportWidth < 640 ? 24 : 30;
  const units = Array.from(text).reduce((sum, char) => {
    const code = char.charCodeAt(0);
    return sum + (code >= 0x20 && code <= 0x7e ? 0.62 : 1);
  }, 0);
  return Math.min(
    Math.max(80, Math.ceil(units * fontSize + 32)),
    Math.max(200, Math.floor(viewportWidth * 0.9))
  );
}

function getItemHeight(viewportWidth: number) {
  return viewportWidth < 640 ? 46 : 54;
}

function getDurationMs(widthPx: number) {
...
function chooseLane({
  ...
  const lanes = orderLanes(laneCount, seed);
  for (const delayMs of DELAY_CANDIDATES_MS) {
    const startAt = now + delayMs;
    for (const lane of lanes) {
      const xPx = getLaneX(lane, laneCount, widthPx, viewportWidth);
      const driftX = getLaneDrift(seed, lane);
      if (
